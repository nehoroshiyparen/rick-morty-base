...
import type { EpisodeFull } from "../types/EpisodeFull";

export const useEpisodes = (limit = 10) => {
  const [episodes, setEpisodes] = useState<EpisodePreview[]>([]);
  const [episode, setEpisode] = useState<EpisodeFull | null>(null);
  ...
  return { episode, episodes, loading, error, hasMore, loadMore, loadOne, updateOne };
};
